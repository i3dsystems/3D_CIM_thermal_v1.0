function [Y, C, H] = Matrix_build_inter(chip, system, hc, Cdt_L, Cdt_V, Cap, Den, ...
        ...
        for j = 1:1:gridNy_pack
            ...
            elseif i ~= 1 && i~= gridNx_pack && j ~= 1 && j~=gridNy_pack
                ...
                elseif j == gridNy_pack
                        ...
                        kw_u = Cdt_L(s-1, 1)*y1*lz_u;ke_u = Cdt_L(s-1, 1)*y1*lz_u;
    ...
    box.inter = zeros(system.chip.N, 4);
    
    for k = 1 : 1 : system.chip.N               
        box.hsp(k, :) = ChipInPlane(chip(k), system.hsp);
        box.inter(k, :) = ChipInPlane(chip(k), system.inter);
    end
    ...
    t_check=tic;
    for k = 1 : 1 : system.chip.N       
        chip_xmesh = chip(k).Xmesh;
        chip_ymesh = chip(k).Ymesh;    
        gridNx_chip = chip(k).Nx;
        gridNy_chip = chip(k).Ny;
        start = sum(Layer.N(1:k-1))-(k-1)+Layer.hsp+1;
            ...
            for i = 1:1:gridNx_chip
                for j = 1:1:gridNy_chip
                    ...
                    if layer_id == Layer.N(k)+start-1   
                        temp = box.inter(k,:);
                        xl = temp(1); xr = temp(2); yb = temp(3); yt = temp(4);                        
                        b = (i-1+xl)+(j-2+yb)*system.inter.Nx + system.Nchip;
                    else
                    ...
                    if( ( i==1 || i==gridNx_chip ) && ( j==1 || j==gridNy_chip ))
                        if i == 1 && j == 1
                                ...
                                row(pointer)=id; column(pointer)=id; value(pointer, 1)=temp;
                                value(pointer, 2) = 1/8 *lz_d * ( Den(id)*Cap(id)*x2*y2); 
                                value(pointer, 2) = 1/8 *lz_u * ( Den(t)*Cap(t)*x2*y2) + value(pointer, 2);
                                value(pointer, 3) = 1/2*(h*y2*lz+h*x2*lz);
                                pointer=pointer+1;                    
                        ...
                        elseif i == gridNx_chip && j == 1
                                ...
                                row(pointer)=id; column(pointer)=id; value(pointer, 1)=temp;
                                value(pointer, 2) = 1/8 *lz_d * ( Den(id-1)*Cap(id-1)*x1*y2); 
                                value(pointer, 2) = 1/8 *lz_u * ( Den(t-1)*Cap(t-1)*x1*y2 ) + value(pointer, 2);
                                value(pointer, 3) = 1/2*(h*y2*lz+h*x1*lz);
                                pointer=pointer+1;                      
                        ...
                        elseif i == 1 && j == gridNy_chip
                                ...
                                row(pointer)=id; column(pointer)=id; value(pointer, 1)=temp;
                                value(pointer, 2) = 1/8 *lz_d * ( Den(s)*Cap(s)*x2*y1);
                                value(pointer, 2) = 1/8 *lz_u * ( Den(t_s)*Cap(t_s)*x2*y1) + value(pointer, 2);
                                value(pointer, 3) = 1/2*(h*y1*lz+h*x2*lz);
                                pointer=pointer+1;  
                                ...
                                row(pointer)=id; column(pointer)=id; value(pointer, 1)=temp;
                                value(pointer, 2) = 1/8 *lz_d * ( Den(s-1)*Cap(s-1)*x1*y1); 
                                value(pointer, 2) = 1/8 *lz_u * ( Den(t_s-1)*Cap(t_s-1)*x1*y1) + value(pointer, 2);
                                value(pointer, 3) = 1/2*(h*y1*lz+h*x1*lz);
                                pointer=pointer+1;                      
                        end
    ...
    fprintf('chip layers done: %f\n', toc(t_check));
    
    pack_xmesh = system.inter.Xmesh;
    pack_ymesh = system.inter.Ymesh;

    gridNx_pack = system.inter.Nx;
    gridNy_pack = system.inter.Ny;
    
    %% top of interposer layer
    box.pack = ChipInPlane(system.inter, system.pack);
    layer_id = layer_id + 1;
    checker = checker + 1;
    t_check=tic;
    for i = 1:1:gridNx_pack
        for j=1:1:gridNy_pack
            lz_d = Thick_layer(layer_id+system.chip.N-1);
            ...
            n = id + gridNx_pack;
            if Layer.inter == 1
                xl = box.pack(1); yb = box.pack(3);
                b = system.Ninter + (xl + i - 1) + (yb - 2 + j)*system.pack.Nx;
            else
                b = id+gridNx_pack*gridNy_pack;
            end

            die_id = box2chip (i, j, box.inter);
            if die_id ~= 0
                [t, lz_u] = pack_id2chip_id(die_id, Layer, system.Nhsp, chip, i, j, box.inter(die_id,:));
                t_s = t - chip(die_id).Nx;
                temp = box.inter(die_id,:);
                xl = temp(1); xr = temp(2); yb = temp(3); yt = temp(4);
            end            
    ...
    fprintf('interposer top layer done: %f\n', toc(t_check));
    %% the other interposer layers
    t_check=tic;
    start_id = layer_id + 1;
    for layer_id = start_id : start_id + Layer.inter - 2
        checker = checker + 1;
        for i = 1:1:gridNx_pack
            for j = 1:1:gridNy_pack
                lz_u = Thick_layer(layer_id+system.chip.N-2);       
                lz_d = Thick_layer(layer_id+system.chip.N-1);
                lz = (lz_u+lz_d)/2;
                ...
                const = gridNx_pack*gridNy_pack*(layer_id-start_id+1)+system.Nchip;
                id = i+(j-1)*gridNx_pack+const;
                w = id-1;
                e = id+1;
                s = id - gridNx_pack;
                n = id + gridNx_pack;         
                if layer_id < start_id + Layer.inter - 2
                    b = id + gridNx_pack*gridNy_pack;
                else
                    xl = box.pack(1); yb = box.pack(3);
                    b = system.Ninter + (xl + i - 1) + (yb - 2 + j)*system.pack.Nx;
    ...
    layer_id = Layer.Nt-Layer.pack+2-system.chip.N;
    checker = checker + 1;
    t_check=tic;
    for i = 1:1:gridNx_pack
        for j=1:1:gridNy_pack
            ...
            if die_id ~= 0
                xl = box.pack(1); xr = box.pack(2); yb = box.pack(3); yt = box.pack(4);
                lz_u = Thick_layer(layer_id+system.chip.N-2);
                t = system.Ninter - system.inter.Nx * system.inter.Ny + ...
                    (i - xl + 1) + (j - yb)*system.inter.Nx;
                t_s = t - system.inter.Nx;
            end            
    ...
    t_check=tic;    
    for layer_id = Layer.Nt-Layer.pack+3-system.chip.N : Layer.Nt+ 1 - system.chip.N
        checker = checker + 1;
        for i = 1:1:gridNx_pack
            for j = 1:1:gridNy_pack
                lz_u = Thick_layer(layer_id+system.chip.N-2);       
                ...
                const = gridNx_pack*gridNy_pack*(layer_id-(Layer.Nt-Layer.pack+2-system.chip.N))+system.Ninter;
                id = i+(j-1)*gridNx_pack+const;
                w = id-1;
                e = id+1;
                s = id - gridNx_pack;
    ...
    %% bottom of the board
    checker = checker + 1;
    layer_id = Layer.Nt+2 - system.chip.N;
    t_check=tic;

    temp = ChipInPlane(system.box, system.pack);  
    xl = temp(1); xr = temp(2); yb = temp(3); yt = temp(4);
    
    for i = 1:1:gridNx_pack
        for j = 1:1:gridNy_pack
            lz_d = 0;
            lz_u = Thick_layer(layer_id-2+system.chip.N);
            lz = (lz_u+lz_d)/2;
            ...
            const = system.Ninter+gridNx_pack*gridNy_pack*Layer.pack;
            id = i+(j-1)*gridNx_pack+const;
            w = id-1;
            e = id+1;
            s = id - gridNx_pack;
