function [Y, C, H] = Matrix_build(chip, system, hc, Cdt_L, Cdt_V, Cap, Den, ...
    ...
    %% top of bridge/package layer
    %because all the TIM layers are merged to one
    layer_id = Layer.Nt-Layer.pack-Layer.bridge+2-system.chip.N;
    checker = checker + 1;
    t_check=tic;
    for i = 1:1:gridNx_pack
        for j=1:1:gridNy_pack
    ...
    t_check=tic;
    for layer_id = Layer.Nt-Layer.pack-Layer.bridge+3-system.chip.N : Layer.Nt+ 1 - system.chip.N
        checker = checker + 1;
        for i = 1:1:gridNx_pack
            for j = 1:1:gridNy_pack
                ...
                const = gridNx_pack*gridNy_pack*(layer_id-(Layer.Nt-Layer.pack-Layer.bridge+2-system.chip.N))+system.Nchip;
                id = i+(j-1)*gridNx_pack+const;
                w = id-1;
                e = id+1;
                s = id - gridNx_pack;
    ...
    for i = 1:1:gridNx_pack
        for j = 1:1:gridNy_pack
            ...
            const = system.Nchip+gridNx_pack*gridNy_pack*(Layer.pack+Layer.bridge);
            id = i+(j-1)*gridNx_pack+const;
            w = id-1;
            e = id+1;
            s = id - gridNx_pack;
