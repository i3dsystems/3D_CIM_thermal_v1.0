...
def Retention(x, t, v, detect, target):
    ...
    if detect == 1: # need to define the sign of v 
        ...
        ratio = t**(v*sign)
        '''
        # use exponential model. Only one direction: Ron->Roff
        truncateX = (x+1)/2
        ratio = torch.exp((torch.zeros_like(x) + v) * t)
        # print("[%s] ratio:"  % __file__, t, v)
        # print("[%s] ratio:"  % __file__, ratio)
    else :  # random generate target for each cell
        sign = torch.randint_like(x, -1, 2)
        truncateX = (x+1)/2
        ratio = t**(v*sign)
        # print("[%s] ratio:"  % __file__, t, v, sign)
        # print("[%s] ratio:"  % __file__, ratio)
